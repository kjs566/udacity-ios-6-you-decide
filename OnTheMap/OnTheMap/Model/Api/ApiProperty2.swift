...
class ApiObservableProperty2<T: Decodable> : ObservableProperty<T>{
    let id: String?
    
    typealias ChangeCallback = (ApiObservableProperty2<T>)->Void
    ...
    var callbacks : [String: (ApiObservableProperty2<T>)->Void]
    ...
    private func notifyChange(){
        printProperty()
        callbacks.values.forEach { (callback: @escaping (ApiObservableProperty2<T>) -> Void) in
